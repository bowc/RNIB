...
Venda.Attributes.Initialize = function() {
	if(!Venda.Ebiz.isTouchDevice()) Venda.Attributes.initViewLargeImagePopup();
		if(Venda.Attributes.attsArray.length > 0) {
			...
			var url = jQuery("#tag-ebizurl").text() + '/content/ebiz/' + jQuery("#tag-ebizref").text() + '/resources/js/mobile/' + attributesUI + '.js';
		...
		Venda.Attributes.ImageSwap("");
	if (jQuery(jQuery("#alternativeImgID").text()).find("a").length > 1 && jQuery('#addedmsg').length == 0) jQuery('.enlarge').fadeIn(3000).fadeOut(5000);
};
			...
			stockFeedbackBox.addClass("Out_of_stock_box");
				...
				if (emwbisType == 'none'){
					...
					if (emwbisType == 'release' && Venda.Attributes.HasReleaseDate(index)){
						...
						addToBasketLinks.button(); //for mobile
					}
				}
	...
	for(var i = 0; i < Venda.Attributes.productArr.length; i++) {
		if (Venda.Attributes.productArr[i].attSet.id == uID) {
			...
			for (var t = 0; t < Venda.Attributes.productArr[i].attSet[attributeNumber].options.length; t++) {
				swatchList += '<li class="'+ clearText('swatch'+Venda.Attributes.productArr[i].attSet[attributeNumber].options[t]) +' attributeSwatch" id="attributeSwatch_' + uID + '" data-attName="'+ attributeNumber +'" data-attValue="'+ Venda.Attributes.productArr[i].attSet[attributeNumber].options[t] +'"><span class="swatchText">' + Venda.Attributes.productArr[i].attSet[attributeNumber].options[t] + '</span></li>';
			}

			//for mobile - a <select> tag need to remove (preventing jquery mobile css rendered)
			jQuery("#oneProduct_" + uID + " ."+attributeNumber+" div.ui-select").replaceWith("<ul id='swatchList_" + attributeNumber + "'>" + swatchList + "</ul>");
		}
	}
};
...
Venda.Attributes.generateDropDowns = function(attributeNumber, uID) {
	...
	for(var i = 0; i < Venda.Attributes.productArr.length; i++) {
		if (Venda.Attributes.productArr[i].attSet.id == uID) {
			var options = '<option value="" data-placeholder="true">' + jQuery("select[name='" + attributeNumber + "'] option:first").text() + '</option>';
	...
	jQuery("#oneProduct_" + uID + " select").selectmenu('refresh','true');
};
...
Venda.Attributes.PresetAtt = function(index, uID) {
	...
	if(allAttsOne >= Venda.Attributes.HowManyAtts(uID)) {
		...
		// Alby - if there is only one value for an attribute, then pre select it
		/*	jQuery("select[id='att"+ o +"_" + uID + "'] option[value='" + Venda.Attributes.productArr[index].attSet["att" + o].options[0] + "']").attr('selected', 'selected');
			...
			jQuery("select[id='att"+ o +"_" + uID + "']").selectmenu('refresh');*/
		}
	}
};
...
Venda.Attributes.updateAttributes = function (uID, what, param) {
	...
	var strname = Venda.Attributes.productArr[0].attSet.att1.name;
	strname = strname.toLowerCase();
	var onlyColour = new RegExp("^(colour|color)$");

	if(onlyColour.test(strname) && (Venda.Attributes.storeImgsArr.length > 0) && (Venda.Attributes.productArr[0].attSet.att1.selected != "")) { Venda.Attributes.ImageSwap(Venda.Attributes.productArr[0].attSet.att1.selected); };
};
...
Venda.Attributes.ImageSwap = function(att) {
	...
	if(Venda.Attributes.storeImgsArr.length > 0) {
		var obj;
		...
		Venda.Attributes.ViewAlternativeImg(obj);
		Venda.Attributes.ViewMainImg(obj,0);
	}
};

Venda.Attributes.ViewMainImg = function(obj, current) {
	var objMedium = jQuery(jQuery("#mainImgID").text());
	var objAltview = jQuery(jQuery("#alternativeImgID").text());
	var mediumImg = "";

	if(objAltview.html()!="") {
		objAltview.find("a").each(function(){
			if(jQuery(this).attr("itemnum") == current){
				mediumImg = "<img itemnum=\""+current+"\" src=\""+obj.images.imgM[current]+"\">";
			}
		});
	}else{
		mediumImg = "<img itemnum=\""+current+"\" src=\""+Venda.Attributes.storeImgsArr[0].noImages.imgM+"\">";
	}
	objMedium.html("").append(mediumImg);
	objMedium.addClass("preload");

	objMedium.find("img").load(function(){
		jQuery(this).unbind("load").show();
		// assign min-height value from an image dimension was loaded
		var height = objMedium.find("img:first").height();
		objMedium.css("min-height", height);
		jQuery(".preload").css("min-height", height);
		objMedium.removeClass("preload");
	}).error(function(){console.warn('Sorry an image cannot be loaded at the moment.');});
};
...
Venda.Attributes.ViewAlternativeImg = function(obj) {
	...
	for(var i = 0; i < obj.images.imgS.length; i++) {
		if(obj.images.imgS[i] != "" && obj.images.imgM[i] != "") {
			altLink += "<a href=\""+obj.images.imgM[i]+"\" itemnum=\""+i+"\" rel=\"external\"><img src=\""+obj.images.imgS[i]+"\"></a>";
		}
	}
	objAltview.html(altLink);

	/* bind event to link */
	objAltview.find("a").bind("click", function(event){
		event.preventDefault();
		objAltview.find("a").removeClass("selected");
		jQuery(this).addClass('selected');
		Venda.Attributes.ViewMainImg(obj, jQuery(this).attr("itemnum"));
	});
	if(objAltview.find("a").length == 1) {
		objAltview.hide();
		jQuery('.enlarge').hide();
	}
	objAltview.find("a:eq(0)").trigger("click");
};

// Events
var swipeEvent = function(){
	jQuery(".prodImages").live("swipeleft swiperight", function(e){
		var objAltview = jQuery(jQuery("#alternativeImgID").text());
		var howManyAlt = parseInt(objAltview.find("a").length);
		var current = parseInt(objAltview.find("a.selected").attr("itemnum"));

		if(howManyAlt <= 1){return false;}
		switch(e.type){
			case "swipeleft":
				++current;
